...
class board{
	...
	public:
		...
		void Print();												
		char Unicode(type_piece type, color_piece color); 		//converts type+color in unicode char
...
void board::Print(){
	for(int i = 7; i >= 0; i--){
		for(int j = 0; j < 8; j++){
			if(j == 0) Serial.print("|__");
			if (thisboard[i][j] != NULL){
				type_piece type = thisboard[i][j] -> GetType();
				color_piece color = thisboard[i][j] -> GetColor();
				Serial.print(this -> Unicode(type,color));
			}
			else Serial.print("  ");
			if(j != 7) Serial.print("__|__");
			else {Serial.print("__|\t");Serial.print(i+1);}
		}
		Serial.print("\n");
	}
...
char board::Unicode(type_piece type, color_piece color){

	if(type == rook){	
		if(color == black) return 'T';
		if(color == white) return 'T';
	}
	if(type == knight){	
...
bool board::Move(bool scan [8][8], bool scan2 [8][8]){ 					
	...
	Serial.print("\n *****\nError e count "); Serial.print(count); Serial.print("\n ******\n ");
